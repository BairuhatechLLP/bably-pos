...
export class InvoiceItem extends Model<InvoiceItem> {
  ...
  @Column(DataType.DECIMAL(11, 2)) discount_amount: number;
  ...
  @Column(DataType.DATE) paymentdate: Date;
  @Column(DataType.BIGINT) createdBy: number;
  @Column(DataType.BIGINT) companyid: number;
  @Column(DataType.BIGINT) productLocationRef: number;

  @Column(DataType.BIGINT)
  parcel_quantity: number;

  @Column(DataType.BIGINT)
  dine_in_quantity: number;

  @Column(DataType.BIGINT)
  parcel_charge_per_item: number;
