...
@Injectable()
export class CreateCounterInterceptor implements NestInterceptor {
  constructor(
  ...
  async intercept(
    ...
    if (!token || token.length <= 10) {
      ...
      try {
        ...
        //getting the number of counters the user already have from billingtable
        const counters = await this.billingCounter.count({
          where: { adminid: decoded?.userId },
        });
        //if the number of counters user has is more than or equal to his subscription limit then reject
        if (counters >= subscriptionDetails?.counter) {
          throw new ServiceUnavailableException(
            `You can only add ${subscriptionDetails?.counter} counters. Use add-on to add more counters.`
          );
        }
      } catch (err) {
