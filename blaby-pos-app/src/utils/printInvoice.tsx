...
const PrintInvoice = async (product: any, Auth: any) => {
  try {
    ...
    await BluetoothEscposPrinter.printColumn(
      ...
      [
        `Token: ${product?.tokenNo || 0}`,
        '',
        `Table: ${product?.table_details?.table_number || 0}`,
      ],
      {},
    );
      ...
      await BluetoothEscposPrinter.printColumn(
        ...
        [
          ...
          `${item?.quantity}    ${Number(
            item?.productMaster?.sp_price,
          )?.toFixed(2)}`,
          Number(
            Number(item?.productMaster?.sp_price) * Number(item?.quantity),
          ).toFixed(2),
        ],
        {},
      );
